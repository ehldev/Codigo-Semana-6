let listaPlatillos = [
  ...
  },
  {
    ...
    descripcion: "Albondigas de carne de res condimentandas con finas hierbas",
    precio: 18.0,
    stock: 8,
    imagen:
...
function listar() {
  listaPlatillos.forEach((item) => {
    let tarjeta = document.createElement("div");
    tarjeta.classList.add("tarjeta");
    tarjeta.setAttribute("itemId", item.id);
...
function generarTabla() {
  ...
  carritoLocalStorage.forEach((producto) => {
    ...
    let item = listaPlatillos.find((p) => p.id == producto.id);

    tr.innerHTML = `
        <td class="text-sm">${item.nombre}</td>
        <td>${producto.cantidad}</td>
        <td>${item.precio}</td>
        <td>${producto.cantidad * item.precio}</td>
    `;
...
function asignarCarrito() {
  ...
  carritoVacio.classList.add("text-center");
  carritoVacio.textContent = "Aún no ha agregado productos a su carrito";
...
function agregarACarrito(id) {
  ...
  let item = carrito.find((item) => parseInt(item.id) === parseInt(id)) || null;
